...
return function(ctx)
	...
	local function zellij_session_from_cmdline(args)
		...
		for i, arg in ipairs(args) do
			if arg == "attach" or arg == "a" then
				...
				if session and not session:match("^%-") then
					return session
				end
			end
		end
	...
	local function scan_descendants(root_pid)
		...
		while #queue > 0 and steps < 500 do
			...
			if not seen[pid] then
				...
				if has_tmux and has_zellij and has_emacs and has_nvim then
					break
				end
	...
	local function zellij_cmd(session, action)
		if not apps.zellij or not apps.timeout then
			return nil
		end
		return shell_quote(apps.timeout)
			.. " 0.12s "
			.. shell_quote(apps.zellij)
			.. " --session "
			.. shell_quote(session)
			.. " action "
			.. action
	...
	local function kitty_cmd(pid, action)
		if not apps.terminal or not apps.timeout then
			return nil
		end
	...
	local function zellij_has_pane_in_direction(session, d)
		...
		if not tab or not tab.id or not tab.tiled_count then
			return nil
		end
		if tab.tiled_count <= 1 then
			return false
		end
		...
		if not focused or not focused.x or not focused.y or not focused.w or not focused.h then
			return nil
		end
	...
	local function try_emacs(d)
		...
		local expr =
			string.format("(let ((target (window-in-direction '%s))) (when target (select-window target) t))", d.emacs)
	...
	local function try_nvim(socket, d)
		local expr = "winnr('"
			.. d.nvim
			.. "') == winnr() ? 0 : (win_gotoid(win_getid(winnr('"
			.. d.nvim
			.. "'))) ? 1 : 0)"
	...
	ctx.smartFocus = function(direction)
		...
		local has_tmux, has_zellij, has_emacs_proc, has_nvim_proc, zellij_session, nvim_pid =
		...
		if is_terminal and hasNeovim and not has_tmux and not has_zellij then
			if kitty_classes[class] == true and try_nvim(nvim_kitty_socket(pid), d) then
				return
			end
