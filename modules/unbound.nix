...
let
  hostsBlocklist = pkgs.stdenv.mkDerivation {
    ...
    src = pkgs.fetchurl {
      ...
      sha256 = "1ap8dm1lbgvhkh0wf8ahagi2r4iipg6yz1yzdhsl63j6fs8nlgwd";
    };
...
in
{
  ...
  services.prometheus.exporters.unbound = {
    enable = lib.mkIf config.services.unbound.enable true;
    port = 9167;
    openFirewall = true;
  ...
  services.unbound = {
    enable = lib.mkIf config.modules.homelab.unbound true;
    ...
    settings = {
      remote-control = {
        control-enable = true;
        control-port = 8953;
        # Simplest: disable TLS for local polling if your exporter supports it:
        control-use-cert = "no";
      };
